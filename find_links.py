...
mysql_user = "wsl_offline"
mysql_pass = "WSL_WIKI_OFFLINE"

local_server = "http://localhost:8080/mediawiki/"


def get_app_path():
    return os.path.abspath(os.path.dirname(__file__))

pdf_config = pdfkit.configuration(wkhtmltopdf=os.path.join(get_app_path(), 'wkhtmltopdf.exe'))
...
linkdb = SqliteDict(get_app_path() + "/links.db",
                    encode=json.dumps, decode=json.loads,
                    autocommit=False)

# Connect to mysql database to find links
mysqldb = pymysql.connect(host=mysql_server, user=mysql_user, password=mysql_pass,
...
with mysqldb.cursor() as cursor:
    ...
    for key, value in linkdb.iteritems():
        ...
        try:
            ...
            new_url = local_server + "dl_files/" + hash + extension
    ...
    for row in rows:
        ...
        url = local_server + "index.php/" + title
        params = dict()
        params["action"] = "purge"
